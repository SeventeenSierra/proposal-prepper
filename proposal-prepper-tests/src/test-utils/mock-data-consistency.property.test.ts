		...
		it("should generate valid analysis results for all compliance statuses", () => {
			...
			fc.assert(
				fc.property(fc.constantFrom(...complianceStatuses), (status) => {
					...
					expect(analysisResults.analysisMetadata.completedAt).toBeInstanceOf(
						Date,
					);
			...
			fc.assert(
				fc.property(
					...
					(_seed) => {
						...
						expect(analysisResult.analysisMetadata.totalPages).toBeGreaterThan(
							0,
						);
						expect(
							analysisResult.analysisMetadata.processingTime,
						).toBeGreaterThan(0);
						expect(
							Array.isArray(analysisResult.analysisMetadata.rulesChecked),
						).toBe(true);
						expect(
							analysisResult.analysisMetadata.rulesChecked.length,
						).toBeGreaterThan(0);
				...
				fc.property(
					...
					(uploadStatus, analysisStatus, complianceStatus) => {
						...
						const analysisResults =
							generateMockAnalysisResults(complianceStatus);
						...
						expect(deserializedAnalysis.progress).toBe(
							analysisSession.progress,
						);

						expect(deserializedResults.sessionId).toBe(
							analysisResults.sessionId,
						);
						expect(deserializedResults.status).toBe(analysisResults.status);
						expect(deserializedResults.overallScore).toBe(
