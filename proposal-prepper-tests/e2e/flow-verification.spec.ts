...
test('Full Proposal Analysis Flow Verification', async ({ page }) => {
    ...
    const newCheckButton = page.getByRole('button', { name: 'New Compliance Check' });
    ...
    const startAnalysisButton = page.getByRole('button', { name: 'Start Compliance Analysis' });
    ...
    for (const stepName of steps) {
        ...
        const stepContainer = page.locator('div.flex.gap-3').filter({ hasText: new RegExp(stepName, 'i') }).first();
