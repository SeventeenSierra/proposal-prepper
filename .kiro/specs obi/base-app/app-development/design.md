# Design Document - Phase 0: Foundational Base (AWS/FAR)

## Overview

The Proposal Prepper (Contract Checker) is the **Foundational Base** for a larger architectural evolution toward sovereign Object-Based Intelligence. 

### Current Implementation (Pre-Integration MVP)
At present, the application is a Next.js/FastAPI implementation that performs "Direct LLM Analysis". It extracts text from PDFs stored in S3/MinIO and sends it directly to AWS Bedrock (Claude 3 Sonnet) or a local Ollama instance with a generic prompt. **It currently lacks the retrieval-augmented (RAG) capabilities and specialized agents required for high-fidelity compliance checking.**

### Target Architecture (Phase 0 Completion)
The goal for Phase 0 is to implement the "Federated Mesh" architecture shown in the research diagram. This involves transitioning from direct LLM calls to a system of specialized agents coordinated via the Strands Agents SDK, backed by dedicated Knowledge Bases.

![Solution Architecture](file:///Users/afla/.gemini/antigravity/brain/50c7bd5a-153f-4e79-b38b-3885e6ef07cc/uploaded_image_1766803408128.png)
*Figure 1: Target Solution Architecture for the Foundational Base*

### Local Sovereignty Mapping (Development & Sovereign End-State)
To ensure data sovereignty and parity between Local and Cloud environments, the following technological mappings are established for Phase 0:

| Component | Cloud (AWS) | Local (Mac M4 / GTR9) |
| :--- | :--- | :--- |
| **Intelligence Core** | Amazon Bedrock (Nova Pro/Claude) | Ollama (Llama 3 / Gemma 2) |
| **Storage (Object)** | Amazon S3 | MinIO / Local Filesystem |
| **Storage (Vector)** | Amazon OpenSearch | **PostgreSQL + pgvector** |
| **Inference Framework** | Strands Agents SDK | Strands Agents SDK |
| **Ingestion** | AWS Lambda (EO Crawler) | Python/Go Background Jobs |

### Specialized Agent Definitions
To achieve the vision in Figure 1, the following agents must be mapped and implemented:

1.  **FAR Agent**: Specialized in Federal Acquisition Regulation. Uses a dedicated RAG index of the latest FAR/DFARS subparts.
2.  **EO Agent**: Specialized in Executive Orders. Uses a dedicated RAG index populated by the National Archives EO Crawler.
3.  **Technical Agent**: Specialized in proposal-specific technical requirements and past performance validation.
